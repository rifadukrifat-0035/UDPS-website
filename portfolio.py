import streamlit as st
import pandas as pd
import plotly.express as px
import folium
from streamlit_folium import st_folium
from streamlit_extras.card import card
from streamlit_image_comparison import image_comparison
import json

# ---------------------------------------------------------
# ১. হেডার এবং ইন্ট্রো
# ---------------------------------------------------------
st.header("Our Portfolio: Case Studies")
st.markdown("আমাদের ডেটা-চালিত সমাধানগুলি কিভাবে বাস্তব-বিশ্বের শহুরে চ্যালেঞ্জগুলি সমাধান করেছে তা দেখুন।")

st.markdown("### Project Showcase")

# ---------------------------------------------------------
# ২. প্রজেক্ট কার্ড সেকশন
# ---------------------------------------------------------
col1, col2 = st.columns(2)

with col1:
    card(
        title="প্রজেক্ট: স্মার্ট ট্রানজিট সিস্টেম",
        text="শহরের ট্র্যাফিক প্রবাহ অপ্টিমাইজ করতে একটি নতুন বাস রুট সিস্টেমের মডেলিং।",
        image="https://images.unsplash.com/photo-1570125909232-eb263c188f7e?q=80&w=1000&auto=format&fit=crop",
        url="#"
    )

with col2:
    card(
        title="প্রজেক্ট: গ্রিন স্পেস অ্যানালাইসিস",
        text="জনসংখ্যার ঘনত্ব অনুযায়ী নতুন পার্ক এবং সবুজ স্থানের জন্য সর্বোত্তম স্থান সনাক্তকরণ।",
        image="https://images.unsplash.com/photo-1496568813655-eb5e63354007?q=80&w=1000&auto=format&fit=crop",
        url="#"
    )

st.markdown("---")

# ---------------------------------------------------------
# ৩. ইমেজ কম্পারিজন (বিফোর/আফটার)
# ---------------------------------------------------------
st.subheader("কেস স্টাডি: আরবান রিনিউয়াল (আগে ও পরে)")
st.write("স্যাটেলাইট ইমেজ ব্যবহার করে গত ৫ বছরে শহুরে সবুজায়নের প্রভাব।")

try:
    # লোকাল ফাইল খোঁজার চেষ্টা
    image_comparison(
        img1="assets/before_development.png",
        img2="assets/after_development.png",
        label1="2019 (Before)",
        label2="2024 (After)",
        use_container_width=True
    )
except:
    # ফাইল না থাকলে অনলাইন ডেমো
    st.info("ℹ️ ডেমো মোড: লোকাল ইমেজ পাওয়া যায়নি।")
    image_comparison(
        img1="https://www.webfx.com/blog/images/cdn/cdn-original.jpg",
        img2="https://www.webfx.com/blog/images/cdn/cdn-optimized.jpg",
        label1="Original Image",
        label2="Processed Image",
        use_container_width=True
    )

st.markdown("---")

# ---------------------------------------------------------
# ৪. GIS ম্যাপ সেকশন (Folium)
# ---------------------------------------------------------
st.subheader("ইন্টারেক্টিভ GIS ম্যাপ: জোনিং অ্যানালাইসিস")
st.write("ম্যাপে ক্লিক করে জোনিং তথ্য দেখুন (ডেমো)।")

# ম্যাপ তৈরি (নিউ ইয়র্ক ফোকাস, আপনি চাইলে ঢাকা করতে পারেন: location=[23.8103, 90.4125])
m = folium.Map(location=[40.7128, -74.0060], zoom_start=12)

# GeoJSON লেয়ার যোগ করার চেষ্টা
try:
    with open('assets/zoning.geojson') as f:
        geojson_data = json.load(f)
    
    folium.GeoJson(
        geojson_data,
        name="Zoning Districts",
        # aliases ঠিক করা হয়েছে
        popup=folium.GeoJsonPopup(fields=["zone_type"], aliases=["Zone: "]) 
    ).add_to(m)
except:
    pass # ফাইল না থাকলে কিছু করার দরকার নেই, শুধু ম্যাপ দেখাবে

# কাস্টম মার্কার যোগ
folium.Marker(
    [40.7128, -74.0060], 
    popup="প্রস্তাবিত ডেভেলপমেন্ট জোন A", 
    tooltip="Zone A"
).add_to(m)

# Streamlit-এ ম্যাপ রেন্ডার
map_data = st_folium(m, width=700, height=500, use_container_width=True)

if map_data and map_data.get('last_object_clicked_popup'):
    st.write(f"📍 আপনি ক্লিক করেছেন: **{map_data['last_object_clicked_popup']}**")

st.markdown("---")

# ---------------------------------------------------------
# ৫. ডেটা ভিজ্যুয়ালাইজেশন (Plotly Chart)
# ---------------------------------------------------------
st.subheader("ডেমোগ্রাফিক ট্রেন্ড অ্যানালাইসিস")

# ডেটা লোড অথবা তৈরি করার লজিক
try:
    # প্রথমে CSV খোঁজার চেষ্টা
    df = pd.read_csv("assets/population_data.csv")
except:
    # CSV না থাকলে ডামি ডেটা তৈরি করা (যাতে চার্ট দেখায়)
    st.info("ℹ️ ডেমো ডেটা ব্যবহার করা হচ্ছে (CSV ফাইল পাওয়া যায়নি)।")
    data = {
        "Year": [2019, 2020, 2021, 2022, 2023] * 3,
        "District": ["Zone A"]*5 + ["Zone B"]*5 + ["Zone C"]*5,
        "Population": [1000, 1200, 1500, 1800, 2200, 
                       800, 900, 950, 1100, 1300,
                       2000, 1900, 1850, 1800, 1750]
    }
    df = pd.DataFrame(data)

# চার্ট তৈরি
fig = px.line(df, x="Year", y="Population", color="District", 
              title="গত ৫ বছরে বিভিন্ন জোনের জনসংখ্যা বৃদ্ধি", markers=True)

st.plotly_chart(fig, use_container_width=True)